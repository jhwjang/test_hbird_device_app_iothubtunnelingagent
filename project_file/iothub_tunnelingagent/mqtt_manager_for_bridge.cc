...
const std::string WILLING_TOPIC("HC");
const char* LWT_PAYLOAD = "hummingbird last willing";

MQTTManager::MQTTManager(int maxChannel, std::string version, std::string address,
...
void MQTTManager::init(const std::string& path)
{
	...
	if (MQTTHandler_ != nullptr)
	{
		...
		MQTTHandler_->MQTT_Init(gMaxChannel, gFirmwareVersion, path); // 2018.10.29 hwanjang - add version
	}
}
